...
검증 항목:
  1. ensure_schema: 멱등
  2. upsert_candidate: INSERT/UPDATE, UNIQUE(date_kst, ticker)
...
class StockInsightsTest(unittest.TestCase):
    ...
    def test_upsert_candidate_insert_then_update(self):
        upsert_candidate(
            self.con, "2026-07-01", "005930", "삼성전자",
        ...
        row = self.con.execute(
            ...
            "FROM telegram_stock_insights WHERE date_kst=? AND ticker=?",
            ("2026-07-01", "005930"),
        ).fetchone()
    ...
    def test_upsert_candidate_does_not_clobber_existing_analysis(self):
        ...
        update_analysis(self.con, "2026-07-01", "005930", "분석결과 텍스트")
        ...
        row = self.con.execute(
            "SELECT analysis, mention_channels FROM telegram_stock_insights WHERE date_kst=? AND ticker=?",
            ("2026-07-01", "005930"),
        ).fetchone()
    ...
    def test_update_analysis_only_touches_analysis(self):
        ...
        update_analysis(self.con, "2026-07-01", "005930", "분석완료")
        row = self.con.execute(
            "SELECT discovery_reason, analysis FROM telegram_stock_insights WHERE date_kst=? AND ticker=?",
            ("2026-07-01", "005930"),
        ).fetchone()
