"""당일 거래량 상위 랭킹 스냅샷 배치 (키움 ka10030 → watchlist.duckdb).
...
ENV_PATH = ROOT / ".env"
DEFAULT_DB_PATH = Path(__file__).resolve().parents[1] / "db" / "watchlist.duckdb"
...
# ── DB upsert ────────────────────────────────────────────────────────────────
def upsert_ranking(con: duckdb.DuckDBPyConnection, date: str, rows: list[tuple]) -> int:
...
def _is_holiday_stale(con: duckdb.DuckDBPyConnection, date: str, rows: list[tuple]) -> str | None:
...
def upsert_watchlist(con: duckdb.DuckDBPyConnection, date: str, tickers: list[str]) -> int:
    """해당 날짜 행 전체 삭제 후 재삽입 (재실행 시 구성 변동 잔재 방지)."""
...
def run_candidates(wl_db_path: Path | str, krx_db_path: Path | str, date: str) -> list[str]:
    ...
    con = duckdb.connect(str(Path(wl_db_path)))
    try:
...
def main() -> None:
    ...
    parser.add_argument("--db-path", type=Path, default=None, help="watchlist.duckdb 경로")
