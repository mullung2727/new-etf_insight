...
이 저장소는 CodeRabbit 리뷰를 쓴다. CodeRabbit은 PR 이벤트로만 돌기 때문에,
`main`에 직접 커밋하면 코드가 리뷰 없이 들어간다.

- `main`에 직접 커밋하지 않는다. 항상 브랜치를 따고 PR로 올린다.
- 브랜치를 따기 전에 `git log origin/main..main`으로 안 올라간 커밋이 있는지 본다.
  남아 있으면 그 커밋들이 내 PR에 딸려 들어가 함께 리뷰·머지된다(2026-09-03 PR #2 사례).
  먼저 `git push origin main`으로 정리하고 브랜치를 딴다.
- 커밋은 주제별로 쪼갠다. 한 PR에 여러 주제가 들어가도 되지만 커밋은 섞지 않는다.
- push는 사용자가 한다. 에이전트는 커밋까지만 하고 `! git push ...`를 안내한다.
- CodeRabbit 지적은 그대로 수용하지 않는다. 먼저 세 가지를 확인한다.
  1. 이 PR의 diff 안에 있는 코드인가 (diff 밖 기존 코드까지 지적한다)
  2. 지적이 전제한 사실이 맞는가 (스키마 타입, 바인딩 주소, 호스트 해석 여부 등)
  3. 의도된 설계를 문맥 없이 지적한 것은 아닌가
  확인 없이 "맞다"고 답하지 않는다. 안 고친 건은 커밋 메시지에 사유를 남긴다.
- 머지 후 로컬 `main`은 `git fetch --prune` 뒤 `origin/main` 기준으로 맞춘다.
  rebase 머지는 커밋 해시를 새로 쓰므로 로컬과 갈라진다.
