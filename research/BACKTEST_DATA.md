...
| 성격 | 위치 | 커밋 |
|---|---|---|
| 성과 검증 안 된 탐색·실패 기록·데이터 지침 | `research/<주제>/` | O |
| **성과가 확인된 유효전략** — 진입/청산 조건, 튜닝된 파라미터, 그 결과 문서 | `research/private/<주제>/` | X (`.gitignore`) |

유효/무효 판단이 애매하면 커밋하지 말고 사용자에게 묻는다.
운용 파라미터 JSON(`etl/scripts/*.json`)도 유효전략이 확정되면 값 자체는 공개 대상이 아니다 —
바꾸기 전에 사용자에게 확인한다.
...
범위     20210802 ~ 최신   (1,254거래일, 2026-09-16 기준). KOSPI+KOSDAQ 전종목, 상장폐지 종목 포함
출처     KRX OpenAPI (data-dbg.krx.co.kr). pykrx 사용 안 함
```

### 적재 결함 점검 — 쓰기 전에 한 번 돌릴 것

2026-09-17 에 **`market_cap`/`list_shrs` 가 날짜 통째로 NULL 인 50일**(20250102~03, 20260320~20260529 중 48일)과
**604행만 들어간 부분 적재일**(20260417)이 발견돼 재적재로 복구했다. 시총 필터를 쓰는 백테스트는 결측일 다음 날
후보가 전부 "통계 없음"으로 빠져 **에러 없이 표본만 조용히 줄어든다**(당시 2026 표본 약 40% 누락).
...
- 거래일당 약 4.5초 (fetch 2.2초 + insert 2.25초). **1년 ≈ 20분, 3년 ≈ 1시간** (2026-09 실측 약 3.8초)
- 멱등. 이미 적재된 거래일은 건너뛰고, 확정 휴장일은 재호출하지 않음
- 중간에 끊겨도 다음 실행이 남은 날짜만 채움
- ⚠️ **과거 구간을 채우면 `stock_names` 가 그 구간 마지막 날의 종목명으로 덮어써진다** (`ensure_ohlcv` 는 이번 실행에서
  받은 마지막 거래일 이름을 upsert). 사명이 바뀐 종목이 옛 이름으로 돌아가므로, 과거 적재 뒤에는 반드시 최신 거래일을
  `build_krx_ohlcv.py --date <최신일> --force` 로 한 번 더 받아 이름을 되돌릴 것
...
## 2b. 체결 가능성 — 상한가 진입은 반드시 제외 (필수)

**봉 가격 = 체결가로 두면 안 된다.** 진입 시점에 상한가에 묶인 종목은 매수 잔량만 쌓여
실제로는 못 산다. 그런데 이런 종목일수록 다음 날 갭상승이 커서, 안 거르면 수익이 전부
여기서 나온다. 분봉·일봉 근사 모두 해당.

```python
prev_close = 진입일 이전 마지막 일봉 close   # krx_ohlcv, date 는 VARCHAR 라 문자열 비교
if entry / prev_close - 1 >= 0.295:          # 가격제한폭 30% - 호가단위 절사 여유
    제외 (매수 불가)
```

- 판정은 **진입 시점 가격**으로 한다. 장중에 상한가를 찍었다 풀린 건은 살 수 있으니 남긴다
- 전일 종가가 없는 표본(신규상장 등)은 판정 불가 — 건수를 따로 보고할 것
- 반대로 **청산 시점 하한가**도 매도 불가다. 청산 규칙에서 같은 방식으로 처리한다

## 2c. 권리락 — 상장주식수 밴드로는 못 잡는다 (필수)

**무상증자·액면분할 권리락일에는 주식수가 아직 그대로다.** 신주 상장이 며칠 뒤이기 때문이다.
그래서 `list_shrs` 전후 비율 밴드(0.67~1.5) 가드는 권리락을 **전혀** 걸러내지 못한다.
가격은 기준가가 조정돼 하루에 −40~−80% 로 찍히므로, 안 거르면 가짜 손실이 표본에 섞인다.

```
실측 (2026-09-18, krx_ohlcv 전 구간 426만 행, 연속 거래일 쌍)
  종가→익일시가 -32% 이하   1,131건   그중 주식수 밴드를 통과하는 것 980건 (87%)
  -40% 이하                 857건
  +35% 이상                 200건
예: 씨젠 20210422 -45.6% · 알테오젠 20200722 -48.1% · 에코프로비엠 20220624 -72.9%
    디앤디파마텍 20251112 -73.2%   — 전부 sh_ratio = 1.00
```

```python
# 가격제한폭이 ±30% 이므로 종가→익일시가가 ±31% 밖이면 물리적으로 불가능하다
on_gap = next_open / close - 1
if abs(on_gap) >= 0.31:
    제외 (권리락 또는 데이터 오류)
```

- 전체 대비 비율은 0.03% 로 작지만 **거래대금 상위 종목에서 터진다**(씨젠·에코프로비엠·알테오젠).
  유니버스가 좁거나 바스켓이 작으면(k=3) 한 건이 평균을 통째로 흔든다
- `close_bet_research/harness.py` 의 `ok_next = _band(sh, nsh)` 에도 같은 구멍이 있다.
  유니버스 974~1,945종목/일 기준 기대 영향은 −0.01%p 수준이라 기존 결론은 바뀌지 않지만,
  **소수 바스켓 실험은 이 가드를 넣고 다시 확인해야 한다**
- 적용례: `research/private/minute_top30/exp40_limit_fullhistory.py` (top30 51,803행 중 27행 제외)

```
실측 사고 (2026-09-17) — 종가베팅 '초기' 백테스트 모집단 기준. 현행 운영 규칙과 다르다.
  현행 규칙 기준 재측정은 아직 안 함 (research/private/close_bet_overnight 참조)
  표본 192건 중 15:19 상한가 19건 (10%)
  익일 09:01 청산  전체 +0.35%  →  상한가 제외 -0.11%   (상한가만 +4.51%, t +2.17)
  TP5/SL3          전체 +0.21%  →  상한가 제외 -0.18%   (상한가만 +3.72%)
...
### 6-1 실계좌 비용 실측 (2026-09-18)

`watchlist.sqlite3 close_bet_orders` 에서 매수·매도가 모두 체결되고 키움 ka10077 net 손익
(`sell_pl_won`)이 들어온 76건으로 역산했다. 총비용 = (매도금 − 매수금) − net손익.

```
건별 비용률(매수금 기준)   중앙 0.2306%
매도측 분해               세금 0.1998% (거래세+농특세)
                         수수료는 분할매도 줄에 종목 합계가 중복 기록돼 합산이 과대 → 건별 중앙값 사용
```

- **고정비(수수료+세금)는 0.23% 수준이다. 0.6% 가정의 나머지 0.37%p 는 슬리피지 여유다.**
- 종가베팅은 진입(종가)·청산(시가) 모두 **단일가 체결**이라 체결가가 전원 동일 → 통상적 슬리피지가 없다.
  따라서 단일가 매매 전략은 0.23~0.35% 로도 판정할 근거가 있다.
- 단, 단일가라도 **대량 주문은 단일가 자체를 움직인다(market impact)**. 저유동 종목
