...
## 한 줄 요약
미체결(ka10075) 조회 + 정정(kt10002) 백엔드/프론트 추가. 취소(kt10003)는 이미 있음.
...
### ka10075 미체결요청 — EP_ACNT (`/api/dostk/acnt`)
...
### S0-1. 백엔드 조회 한 방
- `tr.py`: `TR_PENDING_ORDERS = "ka10075"` 추가 (한 줄)
- `orders.py`: `list_pending() -> list[dict]` 추가 — 고정 파라미터(`all_stk_tp="0", trde_tp="0", stex_tp="0"`)로 호출, `res.data.get("oso", [])` 그대로 반환 (가공 X, dict 리스트)
- `routers/orders.py`: `GET /orders/pending` → `list_pending()` 반환 (response_model 없이 `list[dict]`)
- **검증**: `curl http://localhost:8001/orders/pending` → `oso` 배열 그대로 나오는지

### S0-2. 프론트 표 한 개
- `broker-client.ts`: `listPending: () => get<PendingOrder[]>("/orders/pending")` + `PendingOrder` 타입(느슨하게, `[key]:unknown` 포함 — Holding 패턴 복붙)
- `components/trading/pending-orders.tsx` (신규): `AccountPanel` 축소판 — `load()` + `DataTable`, 컬럼은 종목명/주문번호/미체결수량/주문가/상태 5개만
...
- **배지 카운트는 상시 마운트**: 탭 닫혀도 미체결 건수 보이게 — `FillToast`처럼 탭 헤더에서 `00` 구독해 카운트 갱신, 목록 본체는 탭 열 때만 로드
  - 구조: 탭 헤더(상시) = 배지 / 탭 패널(조건부) = 목록. 카운트 출처는 가벼운 `listPending().length` 또는 `00` 이벤트 누적 — S1-4에서 방식 확정
...
| 파일 | Phase | 변경 |
|------|-------|------|
| `broker/kiwoom/tr.py` | 0,1 | `TR_PENDING_ORDERS`, `TR_ORDER_MODIFY` 상수 |
| `broker/kiwoom/orders.py` | 0,1 | `list_pending()`, `modify_order()` |
| `broker/routers/orders.py` | 0,1 | `GET /orders/pending`, `PATCH /orders/{order_no}` |
| `broker-web/lib/broker-client.ts` | 0,1 | `PendingOrder` 타입, `listPending`, `modifyOrder` |
