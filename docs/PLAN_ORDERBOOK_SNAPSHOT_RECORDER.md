...
3초마다 1행씩 새 SQLite DB에 적재한다. 매매 로직은 건드리지 않는다.

작성 2026-09-07. 구현 전 설계. **이 문서만 읽고 구현 가능하도록 배경·용어를 포함한다.**
...
남은 질문은 **"09:01에 어떻게 파는 게 가장 잘 받는가"** 다. 단, 이 수집으로 답할 수 있는
것과 없는 것을 처음부터 갈라둔다.
...
알려주지 않기 때문이다. 즉 `0B`는 **B군을 해결하지 못하면서 비용만 늘린다**:
이벤트라 3초 스냅샷으로 압축하면 정보가 손실되고(체결 하나하나가 의미), 그대로 저장하면
양이 급증한다.

체결 사실 자체는 이미 1분봉과 `close_bet_orders`(실제 매도 체결가·시각)에 남는다.
나중에 A군 분석에서 "이 시각의 체결 흐름도 같이 봐야겠다"가 되면 같은 구조에 채널만
추가하면 된다(§5의 mode 확장과 동일한 성격).
...
- `refresh`: **`"0"` = 기존 등록 유지하고 추가, `"1"` = 기존 해지 후 등록(기본값)**
...
| D2 | 저장은 3초 간격 스냅샷 1행. 원본 이벤트는 저장하지 않는다 | `run_orderbook_recorder.py:_flush_loop` |
...
| D10 | `0D`는 `grp_no="2"` + `refresh="0"`. 기존 `00` 구독을 깨지 않는다 | `manager.register_orderbook` |
...
| D14 | 대상 종목의 기준일은 **수집일이 아니라 "수집일 직전의 데이터가 있는 날"** 이다. 그 날짜를 실행기록에 남긴다 | `orderbook_symbols.py` 각 resolver |
...
`0D`는 호가가 바뀔 때마다 온다. 개장 직후 활발한 종목은 초당 수십 건이다.

수집창 08:45~09:15 = **1,800초**, 8종목 기준(§3과 동일 전제):

| 저장 방식 | 종목당 행/일 | 8종목 행/일 | 연간 (245거래일) |
|---|---|---|---|
| 원본 전량 (평균 10건/s 가정, 개장 직후는 훨씬 높음) | 약 18,000 | 약 144,000 | 약 7 GB |
| 1초 스냅샷 | 1,800 | 14,400 | 약 741 MB |
| **3초 스냅샷** | **600** | **4,800** | **약 247 MB** |

3초를 고르는 이유는 용량이 아니라 **청산 워커가 3초 주기로 판단하기 때문**이다
(`run_close_bet_exit.py`, `--poll-sec 3`). 시뮬레이션 격자와 실행 격자가 같아야
"3초 더 기다렸으면 얼마였나"가 실제로 재현 가능한 대안이 된다. 사람이 호가창을 보고
누르는 속도로도 3초는 느리지 않다.

**한계(명시):** 스냅샷 사이에 일어난 일은 기록되지 않는다. 호가가 무너졌다 3초 안에
회복하면 남지 않는다. "우리 주문이 정확히 어느 호가에 체결됐나"의 정밀 재구성은
이 데이터로 불가능하다(그건 `0B`가 필요, §1.2).

### D3 보충 — 왜 DuckDB가 아니라 SQLite인가

`minute_bars.duckdb`는 리서치 배치가 단독으로 쓰는 파일이라 DuckDB로 문제없다.
이 DB는 **장중에 워커가 3초마다 쓰는 동시에 리서치가 읽는다.** DuckDB는 단일 writer라
...
`put_nowait` + 경고 로그). 3초 스냅샷이 목적이라 몇 건 유실은 치명적이지 않지만,
전체 채널(`*`)이 아니라 `0D`만 구독해 유입량을 줄이고 큐 크기를 넉넉히 잡는다(§9 R6).
...
1. **`0D` 등록이 기존 `00`(주문체결) 구독을 깨는 경우** — broker의 체결 자동연결
   (`main.py:_fill_sync_loop`)이 멈춘다. D10(별도 `grp_no`, `refresh="0"`)으로 막고
   R4·R5로 검증한다.
...
3초마다 "그 순간 메모리에 있는 최신 호가창"을 쓰는 구조라, **웹소켓이 끊겨도 마지막
호가창이 계속 정상값처럼 기록된다.** 09:00에 끊기면 09:15까지 09:00 호가창이 300행
...
`ts`는 **절대 시각 격자**다. 워커 시작 시점 기준 상대 3초가 아니다.

```
격자 시각 = window_start 부터 snapshot_sec 배수.  08:45:00, 08:45:03, ... , 09:14:57
window_end 는 배타적(exclusive) — 09:15:00 은 기록하지 않는다.
```

워커를 재시작해도 격자가 그대로라 종목 간·날짜 간 시각이 정렬된다. 상대 격자면
재시작 때마다 08:45:01, 08:45:04 식으로 어긋나 비교가 불가능해진다.
...
5. 이전    시작 시 REG 앞에 REMOVE 를 먼저 보내 grp_no="2" 를 통째로 비운다.
   목록    refresh="0" 은 "추가 등록"이라 새 목록으로 REG 해도 이전 종목이 남는다.
   제거    (초안의 "새 목록으로 덮어쓴다"는 설명은 틀렸다.) 남은 종목은 계속 0D 를
           보내와 broker 이벤트 루프에 불필요한 부하를 준다.
              REMOVE(grp_no="2")  →  REG(grp_no="2", refresh="0", 새 목록)
...
- 큐 상한 초과분은 `event_bus.py:61`이 경고 로그를 남기고 버린다. 3초 스냅샷 목적상
...
| 종목 수 | 스냅샷/일 (08:45~09:15, 3초, 600회) | 행/일 | 용량/일 | 연간 |
|---|---|---|---|---|
| 4 (중앙) | 600 | 2,400 | 약 500 KB | 약 123 MB |
| 8 (최대) | 600 | 4,800 | 약 1.0 MB | 약 246 MB |
| 20 (상한) | 600 | 12,000 | 약 2.5 MB | 약 615 MB |

행 크기 추정 약 220 B (52컬럼 대부분 정수 + PK 인덱스).
`symbols.max` 기본값 20은 실질 상한선일 뿐 평소에는 4~8이다.
§2 D2 표와 같은 전제(수집창 1,800초 ÷ 3초 = 종목당 600 스냅샷)를 쓴다.
D15에 따라 신선하지 않은 스냅샷은 행을 쓰지 않으므로, **실제 행 수는 위 값 이하**다.
...
- 3초 동안 갱신이 없어 직전과 동일한 스냅샷이어도 **`max_stale_sec` 이내라면** 그대로
...
**기준일 규약(D14):** 인자 `date`는 **수집일**(오늘 아침)이다. 각 resolver는 그 날짜를
...
- **`close_bet_candidates`가 기본값이다.** 시총·거래대금 필터를 **적용하지 않은** 후보
...
```
  ...
  │  7) 격자 타이머   벽시계 3초 배수마다 flush              (D16)       │
...
1. **수신과 저장을 분리한다.** 수신 태스크는 종목별 두 칸(`cur`/`nxt`)만 갱신하고,
   ...
   3초에 1회다. **격자 직전 값(`cur`)은 그 격자를 flush할 때까지 덮이지 않는다** —
...
| `etl/scripts/run_orderbook_recorder.py` | 워커 본체. SSE 수신 + 3초 flush |
...
| `ops/scheduled-tasks/orderbook-recorder.xml` | 스케줄 작업. 트리거 08:44. `<MultipleInstancesPolicy>IgnoreNew` 포함(D17-6) |
...
| `broker/kiwoom/ws/manager.py` | ① `register_orderbook(codes)` / `unregister_orderbook()` 추가(현재 소켓 참조 보관, REG 응답 대기) ② 재접속 시 `_subscribe`에서 `0D` 재등록 ③ **REAL 수신 즉시 `_recv_ts` 도장** | `grp_no="2"`, `refresh="0"` 고정. `_recv_ts`는 여기서만 찍는다 |
...
`_subscribe`가 현재 `refresh:"1"`(기존 해지 후 등록)로 `00`을 등록한다
(`manager.py:100-106`). `0D`를 같은 `grp_no`·같은 `refresh`로 보내면 `00`이 해지된다.
그래서 D10(별도 `grp_no`, `refresh:"0"`)이고, R4·R5로 테스트한다.
...
| R4 | `0D` 등록이 기존 `00` 등록을 해지하지 않는다 | `register_orderbook` 호출 시 전송 전문이 `grp_no != "1"` 이고 `refresh == "0"` 인지 (전송 소켓 mock) |
...
| R7 | 스냅샷은 `snapshot_sec` 간격으로만 기록된다. 이벤트가 100건 와도 그 창에 1행이다 | 가짜 시계로 3초 창에 이벤트 100건 투입 → 종목당 1행 |
...
| R26 | 격자는 벽시계 고정이고 `window_end`는 배타적이다 | 가짜 시계로 08:45:01에 기동 → 첫 행 `ts`가 08:45:03. 09:15:00 행 없음 |
...
| R32b | 격자 이후 값은 다음 격자에서 쓰인다 | R32에 이어 `flush(t+3)` → 20건 중 최신값으로 1행 |
...
| U3 | `refresh="0"` + 별도 `grp_no`가 실제로 `00`을 보존하는지 | R4는 전문 형태만 검증한다. 첫 실운영일에 체결 자동연결이 정상인지 로그로 확인해야 함 |
| U4 | SSE 큐 포화 시 이벤트 유실 | 워커는 `0D,system`만 구독하고 큐 상한은 2000(D18). 유실은 `event_bus.py:61` 경고 로그로 관측. 3초 스냅샷 목적상 소수 유실은 허용이지만, 로그가 나면 상한을 올린다 |
...
- [ ] **D14 기준일 — mode별 기대 집합으로 대조**한다. "어제 매수분과 일치"가 아니다:
      - `close_bet_candidates`(기본값) → `orderbook_run.source_date` 날짜의 `llm_scores` **전체**와 일치. 시총·거래대금 필터 적용 전이라 실제 매수분보다 많은 게 정상
